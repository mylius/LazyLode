use ratatui::{
    ...
    widgets::{Block, Borders, Clear, Paragraph, Table, Row},
    Frame,
};
...
pub fn render_deletion_modal(frame: &mut Frame, app: &crate::app::App) {
    ...
    if let Some(preview_data) = app.get_deletion_preview() {
        if let Some((_, result, _)) = app.selected_result_tab_index
            .and_then(|idx| app.result_tabs.get(idx))
        {
            let header = Row::new(result.columns.iter()
                .map(|c| c.as_str())
                .collect::<Vec<_>>())
                .style(Style::default()
                    .fg(app.config.theme.accent_color())
            ...
            let rows: Vec<Row> = preview_data.iter()
                .map(|row| {
                    Row::new(row.iter()
            ...
            let widths = vec![Constraint::Percentage(100 / result.columns.len() as u16); result.columns.len()];
